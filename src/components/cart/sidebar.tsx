...
export const CartSidebar = () => {
    ...
    for (let item of cart) {
        subtotal += item.quantity * item.product.valores[item.product.id]
    }
        ...
        <Sheet>
            <SheetTrigger asChild className="p-0">
                <Button className="relative text-white bg-red-600 w-32 h-8">
                    ...
                    {cart.length > 0 &&
                        <div
                            className="absolute w-5 h-5 text-white bg-green-400 rounded-full -right-1 -top-1">
                            {cart.length}
                        </div>
                    }
                </Button>
            ...
            <SheetContent>
                ...
                <div className="flex flex-col items-center text-xs font-bold text-[#002372]">
                    <div>TOTAL</div>
                    <div className="text-2xl w-40 h-10 flex justify-center items-center border border-gray-300">R$ {subtotal.toFixed(2)}</div>
                </div>
